...
static NSString *_YYNSStringMD5(NSString *string) {
    /**
     x代表以十六进制的格式进行数据输出，并且十六进制中的字母用小写字母（即a,b,c,d,e,f）；
     2代表输出数据至少占用2位；
     0代表如果输出数据不够2位的话，在数据前边补0；
     */
...
static void _YYDiskCacheInitGlobal() {
    ...
    dispatch_once(&onceToken, ^{
        _globalInstancesLock = dispatch_semaphore_create(1); /// 信号量1,保证每次只有一条线程访问
        _globalInstances = [[NSMapTable alloc] initWithKeyOptions:NSPointerFunctionsStrongMemory valueOptions:NSPointerFunctionsWeakMemory capacity:0];
    });
}

/**
    全局的NSMapTable: 存放多个YYDiskCache
 */
...
/**
 每一个YYDiskCache都由一个YYKVStorage,一个信号量锁，和一个并发任务队列构成。
 YYKYStorage封装了对SQL、文件的读写操作，信号量保证线程安全，并发队列提升效率。
 */
...
- (instancetype)initWithPath:(NSString *)path
    ...
    if (!self) return nil;
    /**
     YYDiskCache支持在一个app中创建多个缓存实例，每个独立的实例都必须以独立的路径为准，多次创建同一路径的cache,返回的可能是同一个实例。
     YYCache管理了一个全局集合，类型为NSMapTable，所有的DiskCache实例都保存在这个Map中,该集合是线程安全的，由一个单独的信号量机实例保证。 创建DiskCache对象会先从Map中找对应路径的，没有才会创建新的。
     */
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    if (!value) return;
    NSString *filename = nil;
    /// 如果不是YYKVStorageTypeSQLite 检查数据大小是否大于阀值 大于阀值就存成文件
